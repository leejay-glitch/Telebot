...
async def get_updates(token, offset):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"https://api.telegram.org/bot{token}/getUpdates?offset={offset}")
        return response.json()
...
async def main():
    ...
    while True:
        ...
        # logging.info(f"Updates received: {updates}")
        ...
        await asyncio.sleep(0.01)  # Delay to avoid hitting the rate limit
